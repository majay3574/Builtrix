...
import fs from "fs";
import { IPlaywrightWrapper } from "./IPlaywrightWrapper";

export class PlaywrightWrapper implements IPlaywrightWrapper {
  page: Page;
  readonly context: BrowserContext;
  private static newPage: Page;
  ...
  public getNewPage(): Page {
    if (!PlaywrightWrapper.newPage) {
      console.error(
        "New tab is not initialized. Did you forget to call childTab()?"
      );
    }
   ...
   */
  public async type(locator: string, name: string, data: string) {
    await test.step(`Textbox ${name} filled with data: ${data}`, async () => {
      try {
        ...
        console.error(
          `Failed to type in textbox '${name}' with locator '${locator}'. Error: ${error}`
        );
        throw error;
      }
    });
   ...
   * @param {string} data - The data to be typed into the textbox.
   */
  public async fillAndEnter(locator: string, name: string, data: string) {
    await test.step(`Textbox ${name} filled with data: ${data}`, async () => {
      try {
        await this.page.waitForSelector(locator, {
          state: "visible",
        ...
        console.error(
          `❌ Error in fillAndEnter for "${name}" [${locator}]: ${error}`
        );
        throw error;
      }
    });
   ...
   * @param {string} data - The data to be typed into the textbox.
   */
  public async keyboardType(locator: string, data: string) {
    await test.step(`Textbox filled with data: ${data}`, async () => {
      try {
        ...
        console.error(
          `❌ Error in keyboardType for locator '${locator}' with data '${data}': ${error}`
        );
        throw error;
      }
    });
   ...
   * @param {string} data - The data to be typed into the textbox.
   */
  public async typeAndEnter(locator: string, name: string, data: string) {
    await test.step(`Textbox ${name} filled with data: ${data}`, async () => {
      try {
        await this.page.waitForSelector(locator, {
          state: "visible",
        ...
        console.error(
          `Error in typeAndEnter for "${name}" using locator "${locator}" with data "${data}": ${error}`
        );
        throw error;
      }
    });
  ...
  public async click(
    ...
    await test.step(`The ${name} ${type} clicked${
      ...
      try {
        ...
        console.error(
          `Error in click action for "${name}" of type "${type}" with locator "${locator}"${
            description ? ` - ${description}` : ""
          }. ` + `Details: ${error}`
        );
        throw error;
   ...
   */
  public async forceClick(locator: string, name: string, type: string) {
  ...
  public async getInnerText(locator: string): Promise<string> {
    return await test.step(`Get innerText for element: ${locator}`, async () => {
      try {
        ...
        console.error(
          `Failed to get innerText for locator '${locator}': ${error}`
        );
        throw error;
      }
    });
   ...
   */
  public async getTextContent(locator: string): Promise<string> {
    return await test.step(`Get textContent for element: ${locator}`, async () => {
      try {
        ...
        console.error(
          `Failed to get textContent for locator '${locator}': ${error}`
        );
        throw error;
      }
    });
  ...
  public async fetchAttribute(
    ...
    try {
      ...
      console.error(
        `Failed to fetch attribute '${attName}' from '${locator}': ${error}`
      );
      throw error;
    }
  }
   ...
   */
  public async multipleWindowsCount(): Promise<number> {
    try {
   ...
   */
  public async focusWindow(locator: string): Promise<string> {
    try {
      ...
      console.error(
        `Failed to focus on new window after clicking '${locator}': ${error}`
      );
      throw error;
    }
  }
  ...
  public async switchToWindow(
    ...
    try {
      ...
      console.error(
        `Failed to switch to window with title '${windowTitle}': ${error}`
      );
      throw error;
    }
  }
   ...
   */
  public async acceptAlert(data?: string): Promise<void> {
    try {
    ...
    await test.step(`The ${type} ${name} clicked`, async () => {
      try {
        ...
        if (elementCount > 0) {
          ...
          console.warn(
            `Element not found in frame, attempting in main page: ${locator}`
          );
          await this.page.locator(locator).click({ force: true });
        }
      } catch (error) {
  ...
  public async verifyEleinFrame(
    ...
    await test.step(`Verifying the ${name} is present in the frame`, async () => {
      ...
      if (elementCount > 0) {
        try {
          const frameVisible = await frameEle
            .locator("body")
   ...
   * @param name - A descriptive name for the element.
   */
  public async verifyAndClickEleinFrame(
    frameLocator: string,
    locator: string,
    name: string
  ) {
   ...
   */
  public async mouseHoverandClick(
    hoverLocator: string,
    clickLocator: string,
    Menu: string,
    name: string
  ...
  public async selectDropdown(
    ...
    await test.step(`Selecting from dropdown using ${JSON.stringify(
      options
    )}`, async () => {
      ...
      if (options.value) {
        ...
        console.error(
          "No valid option provided. Please specify value, index, or label."
        );
      }
    });
  }

  public async mouseHover(hoverLocator: string, Menu: string) {
  ...
  public async draganddrop(sourceLocator: string, targetLocator: string) {
  ...
  public async doubleClick(locator: string, name: string) {
  ...
  public async verification(locator: string, expectedTextSubstring: string) {
  ...
  public async waitForElementHidden(locator: string, type: string) {
    try {
      await this.wait("minWait");
      await this.page.waitForSelector(locator, {
        state: "hidden",
  ...
  public async validateElementVisibility(locator: any, elementName: string) {
    try {
  ...
  public async uploadMultipleContent(
    fileName1: string,
    fileName2: string,
    locator: any
  ) {
    ...
    if (inputElementHandle) {
      await inputElementHandle.setInputFiles([
        path.resolve(__dirname, fileName1),
  ...
  public async samplefile(locator: string, Path: string) {
  ...
  public async uploadFile(locator: string, Path: string) {
    const filePath = path.resolve(__dirname, Path);
    const inputElementHandle = this.page.locator(locator);
    if (inputElementHandle) {
  ...
  public async isCheckboxClicked(locator: string, name: string) {
  ...
  public async handleAxisCoordinateClick(x_axis: number, y_axis: number) {
  ...
  public async radioButton(locator: string, name: string) {
    await test.step(`Checkbox ${name} is selected`, async () => {
  ...
  public async childTab(locator: string): Promise<void> {
  ...
  public switchToParentPage(): void {
  ...
  public async switchToChildPage(index: number): Promise<void> {
    const pages = this.context.pages();
  ...
  public getById(locator: string): Locator {
    return this.page.locator(`#${locator}`);
  }
  public getByClass(locator: string): Locator {
